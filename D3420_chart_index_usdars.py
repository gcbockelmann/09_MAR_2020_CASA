...
def paint_index_usd(ticker):
    global df
    #
    # Da la orden de graficar todos los puntos de la serie
    ax = df[['Cierre_del_dia_USD']].plot(color ='k', linestyle = '-', figsize =(12,12), linewidth = 1 ) #18,12 es grande
    #
    #
    # Calcula las Moving Average y los suma a la df
    #
    df["MA200"] = df.Cierre_del_dia_USD.rolling(200).mean()
    df["MA100"] = df.Cierre_del_dia_USD.rolling(100).mean()
    df["MA50"] = df.Cierre_del_dia_USD.rolling(50).mean()
    df["MA25"] = df.Cierre_del_dia_USD.rolling(25).mean()
    #
    ...
    title = "Evolucion del Indice " + ticker + " en USD"
    plt.title(title)
    #    
    ...
    location = original_source + "17_CHART_INDEX_USDARS"
    #
    # Slash es la barra
    slash = str("//")
    #
...
def chart_index_usd(ticker12):
    ...
    # Dibuja
    #
    paint_index_usd(ticker12)
    #
    return

#chart_index_usd("^MERV")
